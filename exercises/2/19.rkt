...
;  by the program, so that we could compute the number of ways 
;  to make a British pound, for example.
...
;  that there area five kinds of U.S. coins). 
; It would be nicer to be able to supply a list of coins to be 
...
; It will still have the same form, but it will acess its second
          ...
          (+ (cc amount
              ...
              (cc (- amount
                     (first-denomination
                      coin-values))
                  coin-values)))))
; Define the procedure 'first-denomination',
...
(define (except-first-denomination coin-values)
  (cdr  (reverse coin-values)))

(define (no-more? coin-values)
  (null? coin-values))

(inspect (cc 100 us-coins))

;; I still find the count change procedure very using for some
; reason, but i just guessed this from the names.
; The order of the denominations doesnt seem to matter ('reverse'
;  is used for this), cant articulate why.