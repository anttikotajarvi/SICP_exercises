...
;  chessboard so that no queen is in check from any other (i.e.,
...
; We can formunlate this approach recursively:
...
;  by placing a queen in each row of the k^th column.
; Now filter these, keeping only the positions for which the
;  queen in the k^th column is safe with respect to the other
;  queens.  This produces the sequence of all the ways to place
;  k queens in the first k columns.
; By continuing this process, we will produce not only one
;  solution, but all the solutions to the puzzle.
;
...
; 'queens' has an intereal procedure 'queen-cols' that returns
...
;  row in which to place the queen for the k^th column.
; Complete the program by implementing the representation for
;  sets of board positions, including the procedure
;  'adjoin-position', which adjoins a new row-column position
;  to a set of positions, and 'empty-board', which represents an
;  empty set of positions.
; You must also write the procedure 'safe?', which determines
;  for a set of positions, whther the queen in the k^th column
...
;; List of queens row position in column.
(define empty-board '())
...
(define (safe? k positions)
  (define (get-y x)
    (nth (dec x) positions)) ; nth is 0-based
  (define (test-horizontal k1 k2)
    (= (get-y k1) (get-y k2)))
  (define (test-diagonal k1 k2)
    (= (abs (- (get-y k1) (get-y k2))) (abs (- k1 k2))))
  ;; Test k against all columns
  (define (result-list) ; the sicp lang has some odd behavior so this has to be a function
    (map (lambda (k2)
           (if (= k2 k)
               #f
               (or (test-diagonal k k2) 
                   (test-horizontal k k2))))
